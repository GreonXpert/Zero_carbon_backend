...
const { createClientAdmin } = require("../../../common/controllers/user/userController");
const Notification = require("../../../common/models/Notification/Notification");
const moment = require("moment");
const { emailQueue } = require("../../../common/utils/emailQueue");
const { withTimeout } = require('../../../common/utils/queueUtils');
...
} = require('../../zero-carbon/workflow/dashboardEmitter');
...
} = require('../../../common/utils/emailServiceClient');


const { renderClientDataHTML, renderProposalHTML } = require('../../zero-carbon/workflow/pdfTemplates');
const { htmlToPdfBuffer } = require('../../../common/utils/pdfService');
...
async function updateClientIdReferencesOnActivation(oldClientId, newClientId, actorUserId, actionLabel = 'proposal_accept') {
  ...
  try {
    const ConsultantClientQuota = require('../quota/ConsultantClientQuota');
...
const assignSupportManager = async (req, res) => {
  try {
    ...
    return res.status(200).json({
      success: true,
      message: "Support manager assigned successfully",
      assignment: {
        clientId: client.clientId,
        companyName: client.leadInfo?.companyName || null,
        supportManager: {
          _id: supportManager._id,
    ...
    console.error("[CLIENT CONTROLLER] Error assigning support manager:", error);
    return res.status(500).json({
      success: false,
